...
MongoClient.connect('mongodb://127.0.0.1:27017', function (err, juegos) {
    if (err !== null) {
        console.log(err);
        return err;
    } else {

        db = juegos.db('JuegosDislexia');
    }
});
const app = express();
//app.use(cors());
...
app.get('/api', function (req, res) {
    ...
    res.send({mensaje: 'logueado correctamente',user:req.user});
});
...
app.get('/juegouno', function(req,res){
    db.collection('Completar').find().toArray(function (err, juego) {
        if (err !== null) {
            console.log(err);
            res.send(err);
        } else {
...
app.get('/juegodos', function(req,res){
    db.collection('Animales').find().toArray(function (err, juego) {
        if (err !== null) {
            console.log(err);
            res.send(err);
        } else {
